...
- 서비스 DB: `data/easylaw.sqlite` (사용자, 세션, 설정, 작업, 문서 메타데이터)
- 코퍼스 DB: `data/legal-corpus.sqlite` (판결문·법령 원문, 사전, API 캐시 —
  용량이 큰 참조 데이터 전용, 자동 ATTACH)
- 마스터 키: `data/.master-key`
- 두 SQLite 파일 모두 WAL 모드로 동작합니다.
- 마스터 키는 최초 실행 시 자동 생성되며 DB 안에 저장하지 않습니다.
- `data` 디렉터리는 소유자만 접근할 수 있는 persistent volume에 두세요.
- **두 DB 파일과 `.master-key`는 반드시 함께 백업하세요.** 키를 잃으면
  암호화된 설정을 복구할 수 없습니다. 코퍼스 DB는 잃어도 재수집으로 복구할
  수 있습니다.

구조와 설계 결정은 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)를 참고하세요.
